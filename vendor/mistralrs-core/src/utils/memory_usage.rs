...
/// CPU bytes we treat as usable, in bytes.
///
/// `available` is sysinfo's probe and authoritative on normal platforms; only
/// when it reports 0 (a platform-detection failure, e.g. macOS with sysinfo
/// 0.36.1) do we fall back to `total`.
// ponytail: fallback exists only because some sysinfo platforms probe
// available_memory() as 0 while total_memory() is sane; normal platforms and
// CUDA/Metal paths are untouched. Known ceiling: on 0-reporting platforms the
// fallback may overstate how much memory is actually idle.
fn usable_cpu_memory(available: usize, total: usize) -> usize {
    if available == 0 {
        total
    } else {
        available
    ...
    pub fn get_memory_available(&self, device: &Device) -> Result<usize> {
        match device {
            Device::Cpu => {
                ...
                sys.refresh_cpu_all();
                let available = usize::try_from(sys.available_memory())?;
                let total = usize::try_from(sys.total_memory())?;
                Ok(usable_cpu_memory(available, total))
            }
            #[cfg(feature = "cuda")]
            Device::Cuda(dev) => {
...
mod tests {
    ...
    #[test]
    fn usable_cpu_memory_prefers_nonzero_available() {
        assert_eq!(usable_cpu_memory(4_096, 16_384), 4_096);
    }

    #[test]
    fn usable_cpu_memory_falls_back_to_total_on_zero_probe() {
        assert_eq!(usable_cpu_memory(0, 16_384), 16_384);
    }
}
