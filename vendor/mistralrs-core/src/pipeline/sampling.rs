...
/// Fixed MinerU output-protocol workaround (see PATCH.md): MinerU's layout
/// tokens (`<|box_start|>` etc.) are special tokens, but 0.8.1 only keeps
/// specials for tool/think-tag paths. This crate pins a MinerU-only CLI, so
/// generation must always keep them. EOS/stop tokens never reach completion
/// bytes (Sequence::add_token drops them), so nothing leaks stop delimiters.
pub(crate) const INCLUDE_SPECIAL_TOKENS_IN_GENERATION: bool = true;
...
pub(crate) async fn finish_or_add_toks_to_seq(
    ...
    let completion_bytes = tok_env
        .tok_trie()
        .decode_ext(&[logprobs.token], INCLUDE_SPECIAL_TOKENS_IN_GENERATION);
...
mod tests {
    ...
    const MINERU_LAYOUT_TOKEN: &str = "<|box_start|>";

    // Minimal ByteLevel BPE tokenizer with one MinerU-style special token.
    ...
    #[test]
    fn generation_decode_preserves_mineru_special_tokens() {
        let tokenizer = Tokenizer::from_str(TOKENIZER_JSON).unwrap();
        // Mirror the production construction: the pipeline reads the TokEnv
        // from the llguidance factory built by `build_llg_factory`.
        let factory = build_llg_factory(tokenizer).unwrap();
        let trie = factory.tok_env().tok_trie();

        let special_id = trie.get_special_token(MINERU_LAYOUT_TOKEN).unwrap();
        assert!(trie.is_special_token(special_id));

        // Unpatched upstream behavior (include_special=false): silently dropped.
        assert!(trie.decode_ext(&[special_id], false).is_empty());
        // Patched generation behavior: the MinerU layout protocol token survives.
        assert_eq!(
            String::from_utf8_lossy(
                &trie.decode_ext(&[special_id], INCLUDE_SPECIAL_TOKENS_IN_GENERATION)
            ),
            MINERU_LAYOUT_TOKEN
        );
    }
