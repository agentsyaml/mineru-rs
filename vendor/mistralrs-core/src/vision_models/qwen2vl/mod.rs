...
/// Local row indices `0..len` used to gather per-batch MRoPE positions.
///
/// The gather target is the local input row (batch `i` of `ropeidx_input_ids`),
/// so selectors must stay within `[0, len)` — never the KV-cache global
/// `offset..offset+len`. See the `ponytail:` comment at the call site for the
/// upstream PR #2301 backport rationale.
fn ropeidx_local_indices(len: usize) -> Vec<i64> {
...
impl Qwen2VLModel {
    ...
    pub fn forward(
        ...
        // ponytail: backport of upstream PR #2301 (commit 1b411b1f). 0.8.1
        // built these indices from the KV-cache global offset
        // (`offset..offset+len`) while gathering a local input row, so the
        // first decode step (seq.len == full prompt len, offset > 0) selected
        // index >= row dim and index-select went out of bounds. Local `0..len`
        // keeps the selector strictly below the row length; decode's
        // `input_ids_full.dim == seq.len` makes this range a valid identity
        // selection. Known ceiling: batch>1 mixed multimodal, prefix-cache
        // prompt trim, and future chunked prefill crossing an image region
        // still need the global offset — see vendor PATCH.md item 3.
        for len in &seqlens {
            ropeidx_attn_mask_indices_bs.push(Tensor::from_vec(
                ropeidx_local_indices(*len),
                (*len,),
                input_ids.device(),
            )?);
        }
...
mod tests {
    ...
    /// Production-bound helper: local row indices must never exceed `len`,
    /// even at the old crash point (prefill len 1395, first decode offset 1394
    /// used to emit index 1395 against a dim-1395 row).
    #[test]
    fn ropeidx_local_indices_never_exceed_len() {
        // Old observed failure: len=1395/offset=1394 -> global range
        // [1394, 2789) gathered against a local row of len 1395, so index
        // 1395 (== row dim) was out of bounds. Local indices are always
        // [0, len), max = len-1 < len.
        let len = 1395usize;
        let indices = ropeidx_local_indices(len);
        assert_eq!(indices.len(), len);
        assert!(indices.iter().all(|&i| i < len as i64));
        assert_eq!(indices[0], 0);
        assert_eq!(indices[len - 1], len as i64 - 1);

        // Single-token decode step: len == seq.len, local [0] in bounds.
        assert_eq!(ropeidx_local_indices(1), vec![0]);
    }

    /// Production-bound helper: indices are per-batch local, so different
    /// batch lengths produce their own contiguous 0..len ranges.
    #[test]
    fn ropeidx_local_indices_vary_per_batch_len() {
        for len in [1usize, 7, 256, 1395] {
            let indices = ropeidx_local_indices(len);
            assert_eq!(indices, (0_i64..len as i64).collect::<Vec<_>>());
        }
    }

    /// Pure algebra mirroring production `get_rope_index` + forward decode:
    /// for a header-text + image-pads + tail-text prompt, each generated token
    /// decodes at `offset + mrope_delta` and that equals the full position
    /// formula `max_llm_pos_ids + 1 + k`, with the delta staying constant as
    /// more tokens are appended.
    #[test]
    fn decode_position_matches_full_formula_and_delta_stable() {
        // header text, image grid (t,h,w), tail text — the layout prompt shape.
        let header_len = 12usize;
        let (t, h, w) = (1i64, 35i64, 35i64);
        let llm_grid = (t * (h / 2) * (w / 2)) as usize; // spatial merge 2
        let tail_len = 8usize;
        let total_len = header_len + llm_grid + tail_len;

        // Production: text pos ids are arange(st_idx, len + st_idx); image pos
        // ids are grid + (text_len + st_idx). max_llm_pos_ids tracks the
        // exclusive upper bound text_len + st_idx, capping at the tail:
        // max(t,h/2,w/2) + header_len + tail_len.
        let max_llm_pos_ids =
            header_len as i64 + tail_len as i64 + *[t, h / 2, w / 2].iter().max().unwrap();
        let mrope_delta = max_llm_pos_ids + 1 - total_len as i64;

        for k in 0..8i64 {
            let offset = total_len as i64 + k; // seqlen offset for generated token k
            assert_eq!(
                offset + mrope_delta,
                max_llm_pos_ids + 1 + k,
                "decode offset+delta must equal full position formula at token {k}"
            );
        }
        // delta is independent of k by construction (no k term), so stability
        // is the algebra above; assert it explicitly for a large k too.
        let k = 10_000i64;
        let offset = total_len as i64 + k;
        assert_eq!(offset + mrope_delta, max_llm_pos_ids + 1 + k);
    }
}
