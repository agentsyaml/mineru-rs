...
mod tests {
    ...
    fn args(values: &[&str]) -> Args {
        Args::try_parse_from(std::iter::once("mineru-api").chain(values.iter().copied())).unwrap()
    }
    ...
    fn vlm_transport_boolean_flags_override_the_frozen_environment() {
        ...
        assert!(Args::try_parse_from(["mineru-api", "--vlm-debug"]).is_err());
        assert!(Args::try_parse_from(["mineru-api", "--vlm-allow-remote-images", "1"]).is_err());
    ...
    fn public_bind_flags_are_strict_and_override_environment() {
        ...
        assert!(Args::try_parse_from(["mineru-api", "--public-bind-exposed"]).is_err());
        assert!(Args::try_parse_from(["mineru-api", "--public-bind-exposed", "1"]).is_err());
    }
