...
fn attach_assets(
    ...
    for (index, block) in page.blocks.iter().enumerate() {
        if block.kind.as_str() == crate::BlockKind::TABLE {
            if let Some(content) = attach_table_images(
                block,
                &data_image,
                &mut assets,
                &mut generated_paths,
...
fn attach_table_images(
    ...
    for capture in image.captures_iter(content) {
        ...
        let upper =
            base64_decoded_upper_bound(encoded).ok_or_else(|| asset_limit_overflow(limit))?;
        if upper > *remaining {
            // Resolve a possible duplicate without allocating the decoded asset.
            let (md5, decoded) = streamed_base64_md5(encoded, limit)?;
...
mod tests {
    ...
    fn attach(page: &mut PageResult, image: &RgbImage, limit: usize) -> crate::Result<Vec<Asset>> {
        attach_assets(page, image, 0, limit, &HashSet::new())
    }
    ...
    fn incremental_assets_across_pages_are_cumulative_deduplicated_and_atomic() {
        ...
        for page_index in 0..2 {
            ...
            let generated = attach_assets(&mut page, &image, used, usize::MAX, &paths).unwrap();
